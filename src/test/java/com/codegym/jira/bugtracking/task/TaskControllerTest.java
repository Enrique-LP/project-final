...
import static com.codegym.jira.bugtracking.task.TaskService.CANNOT_ASSIGN;
import static com.codegym.jira.bugtracking.task.TaskService.CANNOT_UN_ASSIGN;
import static com.codegym.jira.bugtracking.task.TaskTestData.*;
...
class TaskControllerTest extends AbstractControllerTest {
    ...
    void getNotFound() throws Exception {
        perform(MockMvcRequestBuilders.get(TASKS_REST_URL_SLASH + UserTestData.NOT_FOUND))
    ...
    void updateTaskWhenProjectNotExists() throws Exception {
        TaskToExt notExistsProjectTo = new TaskToExt(TASK2_ID, "epic-2", "Trees UPD", "task UPD", "epic", "in_progress", "high", null, 4, null, UserTestData.NOT_FOUND, SPRINT1_ID);
        perform(MockMvcRequestBuilders.put(TASKS_REST_URL_SLASH + TASK2_ID)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void updateWhenTaskNotExists() throws Exception {
        ActivityTo notExistsActivityTo = new ActivityTo(ACTIVITY1_ID, UserTestData.NOT_FOUND, UserTestData.USER_ID, null, null,
    ...
    void updateActivityWhenChangeTask() throws Exception {
        ActivityTo changedTaskTo = new ActivityTo(ACTIVITY1_ID, TASK1_ID + 1, UserTestData.USER_ID, null, null,
                "in_progress", "low", "epic", null, null, 3, null);
        perform(MockMvcRequestBuilders.put(ACTIVITIES_REST_URL_SLASH + ACTIVITY1_ID)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void changeTaskStatusNotFound() throws Exception {
        perform(MockMvcRequestBuilders.patch(TASKS_REST_URL_SLASH + UserTestData.NOT_FOUND + CHANGE_STATUS)
                .param(STATUS_CODE, READY_FOR_REVIEW))
                .andDo(print())
                .andExpect(status().isNotFound());
    }
    ...
    void createTaskWhenProjectNotExists() throws Exception {
        TaskToExt notExistsProjectTo = new TaskToExt(null, "epic-1", "Data New", "task NEW", "epic", "in_progress", "low", null, 3, null, UserTestData.NOT_FOUND, SPRINT1_ID);
        perform(MockMvcRequestBuilders.post(REST_URL)
                .contentType(MediaType.APPLICATION_JSON)
                .content(writeValue(notExistsProjectTo)))
                .andDo(print())
    ...
    void createActivityWhenTaskNotExists() throws Exception {
        ActivityTo notExistsTaskTo = new ActivityTo(null, UserTestData.NOT_FOUND, UserTestData.ADMIN_ID, null, null, null,
    ...
    void deleteNotFound() throws Exception {
        perform(MockMvcRequestBuilders.delete(ACTIVITIES_REST_URL_SLASH + UserTestData.NOT_FOUND))
                .andDo(print())
                .andExpect(status().isNotFound());
    }
    ...
    void assignToTask() throws Exception {
        perform(MockMvcRequestBuilders.patch(TASKS_REST_URL_SLASH + TASK1_ID + "/assign")
                ...
                .andDo(print())
                .andExpect(status().isNoContent());
        assertTrue(userBelongRepository.findActiveAssignment(TASK1_ID, TASK, UserTestData.ADMIN_ID, TASK_DEVELOPER).isPresent());
    }

    @Test
    @WithUserDetails(value = UserTestData.ADMIN_MAIL)
    void assignToTaskNotFound() throws Exception {
        perform(MockMvcRequestBuilders.patch(TASKS_REST_URL_SLASH + UserTestData.NOT_FOUND + "/assign")
                .param(USER_TYPE, TASK_DEVELOPER))
                .andExpect(status().isNotFound());
    }
    ...
    void unAssignFromTask() throws Exception {
        perform(MockMvcRequestBuilders.patch(TASKS_REST_URL_SLASH + TASK1_ID + "/unassign")
                ...
                .andDo(print())
                .andExpect(status().isNoContent());
        assertTrue(userBelongRepository.findActiveAssignment(TASK1_ID, TASK, UserTestData.ADMIN_ID, TASK_DEVELOPER).isEmpty());
    }
    ...
    void unAssignFromTaskWithNotPossibleUserType() throws Exception {
        ...
        assertTrue(userBelongRepository.findActiveAssignment(TASK1_ID, TASK, UserTestData.ADMIN_ID, TASK_REVIEWER).isPresent());
    }

    @Test
    @WithUserDetails(value = UserTestData.ADMIN_MAIL)
    void unAssignFromTaskNotFound() throws Exception {
        perform(MockMvcRequestBuilders.patch(TASKS_REST_URL_SLASH + UserTestData.NOT_FOUND + "/unassign")
                .param(USER_TYPE, TASK_REVIEWER))
                .andExpect(status().isNotFound());
    }
